...
struct PerlinFunctor {
  ...
  __device__ float octaveNoise(float x, float y, const float persistence = 0.5) const {
    float total = 0.0;
    float amplitude = 1.0;
...
class PerlinNoiseCuda {
  public:
    PerlinNoiseCuda(const unsigned int seed) {
    ...
    thrust::device_vector<float> generate_heightmap(int32_t octaves, float frequency, glm::vec2 dim) {
      ...
      PerlinFunctor perlinFunctor(
        thrust::raw_pointer_cast(device_permutation.data()),
        (int)dim.x, (int)dim.y, octaves, freq_x, freq_y
      );
    ...
    std::vector<float> generate_normalized_heightmap(int32_t octaves, double frequency, glm::vec2 dim) {
      thrust::device_vector<float> device_results = generate_heightmap(octaves, frequency, dim);
    auto result = thrust::minmax_element(device_results.begin(), device_results.end());
    ...
    //TODO Either reshape to 2D vector or refactor original parlay impl 1D vector
    return heightmap;
